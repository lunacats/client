    ...
    def handleaxis(self, number, value):
        ...
        if number == 0: # left stick's left to right
            ...
            if value < -sensitivity() and (self.lastlcommand != "f"):
                ...
                if self.lastlcommand != "n":
                    self.lastlcommand = "n"
                    print("Left stick FB neutral")
                    self.senddata( b'\xD1')
        elif number == 2:  # right stick's left to right
            if value < -sensitivity() and (self.lastrcommand != "l"):
                self.lastrcommand = "l"
                print("Right stick left")
                self.senddata( b'\xDE')
            elif value > sensitivity() and (self.lastrcommand != "r"):
                self.lastrcommand = "r"
